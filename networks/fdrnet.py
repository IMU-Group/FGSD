...
import math
import numpy as np
from networks.DIM import DIMModel
...
class FDRNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 resume=False):  # 添加 resume 参数
        super(FDRNet, self).__init__()
        
        
        # 如果 resume 为 True，则不加载预训练权重
        if resume:
        ...
        soft_pred_layers = []
        if dropout_2d > 1e-6:
            soft_pred_layers.append(nn.Dropout2d(p=dropout_2d))
        soft_pred_layers.append(nn.Conv2d(pred_planes, 1, 1))
        soft_pred_layers.append(nn.ReLU())
        self.soft_pred_conv = nn.Sequential(*soft_pred_layers)
        
        # 输入bx32xhxw，输出bx1xhxw
        binary_layers = []
        if dropout_2d > 1e-6:
            binary_layers.append(nn.Dropout2d(p=dropout_2d))
        binary_layers.append(nn.Conv2d(pred_planes+1, 1, 1))
        binary_layers.append(nn.ReLU())
        self.binary_layers = nn.Sequential(*binary_layers)
    ...
    def forward(self, x):
        ...
        h_f, w_f = ml_features[0].size()[2:] # h_f和w_f是通过提取多层特征后得到的第一层特征ml_features的高度和宽度
        proj_features = []
        for i in range(len(ml_features)):
            cur_proj_feature = self.proj_convs[i](ml_features[i]) # 对每一层特征ml_features[i]进行1x1投影卷积操作，将其投影到指定的维度上
            cur_proj_feature_up = F.interpolate(cur_proj_feature, size=(h_f, w_f), mode='bilinear')# 通过双线性插值操作，将当前投影后的特征cur_proj_feature上采样至与第一层特征相同的大小
            proj_features.append(cur_proj_feature_up) # 将上采样后的特征添加到proj_features列表中，保存所有处理后的特征。
        cat_feature = torch.cat(proj_features, dim=1) # 将所有处理后的特征在指定的维度上进行拼接，得到cat_feature，这样就将多个尺度的特征合并成了一个更大的特征张量，用于后续的处理。
    
        stem_feat = self.stem_conv(cat_feature) #  1x1卷积得到综合特征
        
        # factorised feature
        inv_feat, var_feat, mix_feat = self.fr(stem_feat) # 对综合特征stem_feat进行分解和重组操作，得到inv_feat, var_feat, mix_feat
        if self.training:
            logits = self.pred_conv(inv_feat)
            soft_feat = self.soft_pred_conv(mix_feat)
            
            binary_mask = self.binary_layers(torch.cat([soft_feat, mix_feat], dim=1))
            binary_mask = F.interpolate(binary_mask, size=(h, w), mode='bilinear')
            
            logits = F.interpolate(logits, size=(h, w), mode='bilinear')
            soft_feat = F.interpolate(soft_feat, size=(h, w), mode='bilinear')
            g = self.fc(var_feat.mean([2, 3]))
            

            return logits, inv_feat, g, soft_feat, binary_mask
        else:
            ...
            return {'logit': logits,'soft_feat': soft_feat,'binary_mask': binary_mask}
...
def create_model_and_optimizer(args):
    ...
    model = FDRNet(backbone='efficientnet-b3',
                   ...
                   resume=args.resume is not None)  # 传递 resume 参数
