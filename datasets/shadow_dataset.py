...
class ShadowDataset(Dataset):
    ...
    def __init__(self, data_root, im_size=512, phase='train', max_dataset_size=None):
        ...
        if phase == 'train':
            self.img_dirs = [self.train_A_dir, self.train_B_dir, self.train_C_dir]
        elif phase == 'test':
            self.img_dirs = [self.test_A_dir, self.test_B_dir, self.test_C_dir]
        elif phase == 'val':
            self.img_dirs = [self.train_A_dir, self.train_B_dir, self.train_C_dir]  # Assuming validation uses training data
    ...
    def __getitem__(self, index):
        ...
        # soft_mask
        soft_mask_name = os.path.splitext(img_name)[0]+'.png'
